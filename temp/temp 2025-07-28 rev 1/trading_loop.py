...
async def trading_logic(trading_client: TradingClient, data_client: StockHistoricalDataClient, symbol: str, current_price: float, log_file_path: str, file_handler, last_trade_time: dict):
    ...
    if use_mock_data:
        ...
        bars = fetch_bar_data(data_client, symbol, timeframe, SEQUENCE_LENGTH, file_handler)  # Sync function
        daily_bars = fetch_daily_data(data_client, symbol, NUM_DAYS_HISTORY, file_handler)  # Sync function
        ...
        for symbol in SYMBOLS:
            ...
            if position != 0:
                ...
                if TRADE_MODE == 'simulation':
                    ...
                    request_params = StockLatestBarRequest(symbol_or_symbols=symbol, feed=DataFeed.IEX)
                    try:
