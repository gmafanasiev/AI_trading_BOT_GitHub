...
def analyze_trades(log_file: str, trading_client=None) -> dict:
    ...
    trade_types = []  # Track trade types (short/long/close)
    ...
    try:
        ...
        for line in lines:
            ...
            if "BUY order" in line:
                ...
                if match and timestamp >= time_threshold:
                    ...
                    trade_times.append(timestamp)
                    trade_types.append('C')  # 'C' for close
        ...
        use_times = trade_times and all(isinstance(t, datetime) for t in trade_times)
        if use_times:
            ...
            plt.xlabel('Time (Short/Long/Close, Trade Number)')
        else:
            plt.plot(range(len(equity_curve)), equity_curve, label='Equity Curve', color='#1f77b4', marker='o')
            for i, (equity, trade_type) in enumerate(zip(equity_curve, trade_types)):
                plt.annotate(f"{trade_type}{i+1}", (i, equity), textcoords="offset points", xytext=(0,10), ha='center')
            plt.xlabel('Trade Number (Short/Long/Close)')
