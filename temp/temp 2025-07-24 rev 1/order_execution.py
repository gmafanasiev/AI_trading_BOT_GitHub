...
async def poll_order_fill(trading_client: TradingClient, order_id: str, timeout: int = 10) -> tuple[float, float]:
...
async def update_position(symbol: str, side: str, trade_type: str, filled_qty: float, filled_price: float, file_handler):
...
async def execute_order(trading_client: TradingClient, symbol: str, qty: float, side: str, trade_type: str, current_price: float, log_file_path: str, file_handler) -> tuple[bool, str]:
    ...
    if TRADE_MODE == 'simulation':
        ...
        success = await update_position(symbol, side, trade_type, qty, current_price, file_handler)
        return success, ""
    else:
        try:
            ...
            success = await update_position(symbol, side, trade_type, filled_qty, filled_price, file_handler)
            return success, ""
