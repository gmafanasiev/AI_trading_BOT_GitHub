...
import logging
import numpy as np
from config import URL, XAI_API_KEY, SEQUENCE_LENGTH, UPPER_THRESHOLD, LOWER_THRESHOLD, TRADE_MODE, TEMPERATURE, MARKET_CORR, SUPPORTS, RESISTANCES, BACKTEST_MODE
...
async def get_x_sentiment(url: str, api_key: str, symbol: str) -> str:
    ...
    """
    if BACKTEST_MODE:
        return "Neutral sentiment, no key updates."  # Mock for backtest
    try:
...
async def get_grok4_prediction_and_adjustments(url: str, api_key: str, stats: dict, symbol: str, trade_summary: dict) -> tuple[float, float, float]:
    ...
    if BACKTEST_MODE:
        ...
        risk_per_trade = 0.01
        logging.info(f"Mocked prediction for {symbol} in backtest: {prediction}")
    ...
    try:
        ...
        prompt = (
            ...
            f"Supports: {SUPPORTS.get(symbol, [])}, Resistances: {RESISTANCES.get(symbol, [])}. "
