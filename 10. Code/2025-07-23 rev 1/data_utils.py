...
from config import SIMULATION_MODE, ALPACA_API_KEY, ALPACA_API_SECRET
...
async def fetch_bar_data(data_client: StockHistoricalDataClient, symbol: str, timeframe, limit: int, file_handler) -> pd.DataFrame:
    ...
    try:
        if SIMULATION_MODE:
            ...
            logging.info(f"Simulated {limit} bars for {symbol}, latest price: ${bars.iloc[-1]['close']:.2f}, market_corr: 0.80, supports: [210.03, 207.22, 199.26], resistances: [213.48, 216.23, 214.65, 213.34]")
            file_handler.flush()
            return bars
        else:
