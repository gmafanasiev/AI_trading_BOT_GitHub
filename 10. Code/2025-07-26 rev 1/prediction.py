...
async def get_grok4_prediction_and_adjustments(url: str, api_key: str, stats: dict, symbol: str, trade_summary: dict) -> tuple[float, float, float]:
    ...
    """
    try:
        headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}  # Define headers here
        ...
        prompt = (
            ...
            f"Volume trend: {np.mean(stats['volume'][-5:]):.0f} (avg last 5), "
        ...
        if match:
            ...
            if prediction not in [0.0, 0.1, 0.2, 0.3, 0.7, 0.8, 0.9, 1.0]:
                logging.warning(f"Invalid prediction {prediction}, defaulting to 0.5")
                prediction = 0.5
        ...
        if "404" in str(e) or "400" in str(e):
            logging.warning(f"Endpoint/model issue. Try model 'grok-3' or check xAI API docs.")
        return 0.5, UPPER_THRESHOLD, 0.01
    except Exception as e:
