...
from data_utils import fetch_bar_data, fetch_daily_data, prepare_grok4_input
...
async def trading_logic(trading_client: TradingClient, data_client: StockHistoricalDataClient, symbol: str, current_price: float, log_file_path: str, file_handler, last_trade_time: dict):
    ...
    qty = min(int((MAX_EQUITY * RISK_PER_TRADE) / current_price), int(max_position_value / current_price))  # Use configured risk

    # Fetch Grok prediction every bar
    timeframe = TimeFrame.Minute
    bars = await fetch_bar_data(data_client, symbol, timeframe, SEQUENCE_LENGTH, file_handler)
    daily_bars = await fetch_daily_data(data_client, symbol, NUM_DAYS_HISTORY, file_handler)
    stats = prepare_grok4_input(bars, daily_bars, SEQUENCE_LENGTH)
    if stats is None:
        logging.warning(f"Skipping Grok prediction for {symbol}: insufficient data")
        file_handler.flush()
        prediction = 0.5  # Default prediction when data is insufficient
        ...
        logging.info(f"Grok prediction for {symbol}: {prediction:.2f} (Position: {position})")
