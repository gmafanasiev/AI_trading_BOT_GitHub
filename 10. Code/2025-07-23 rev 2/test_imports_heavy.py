...
# List of modules and their expected items (updated: no clients.py, added helper in order_execution)
MODULES = {
    'config': ['SYMBOLS', 'SEQUENCE_LENGTH', 'UPPER_THRESHOLD', 'LOWER_THRESHOLD',
               ...
               'TRADE_MODE', 'open_positions', 'sl_prices', 'tp_prices'],
    ...
    'order_execution': ['get_position', 'execute_order', 'update_position_and_levels'],  # Added new helper
...
# Variables that should be imported from config.py (updated for TRADE_MODE)
CONFIG_IMPORTS = {
    ...
    'trading_loop': ['SYMBOLS', 'SEQUENCE_LENGTH', 'UPPER_THRESHOLD', 'LOWER_THRESHOLD',
                     'ADJUSTMENT_INTERVAL', 'MAX_POSITION_PCT', 'MAX_EQUITY',
                     'STOP_LOSS_PCT', 'TAKE_PROFIT_PCT', 'TRAILING_PCT', 'TIMEFRAME',
                     'URL', 'XAI_API_KEY', 'TRADE_MODE', 'open_positions',
                     'sl_prices', 'tp_prices'],
    'order_execution': ['open_positions', 'sl_prices', 'tp_prices', 'STOP_LOSS_PCT',
                        'TAKE_PROFIT_PCT', 'TRADE_MODE']
}
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module == 'config':
                imported_vars.update(name.name for name in node.names)
...
def check_parameters(config_module):
    ...
    for param, expected in expected_params.items():
        ...
        if actual == expected:
            ...
            logger.error(f"Parameter {param:.<50}: Expected {expected}, Found {actual}")
    ...
    if isinstance(adj_interval, int) and adj_interval > 0:
        logger.info(f"Parameter ADJUSTMENT_INTERVAL:.<50: OK (Value: {adj_interval})")
    else:
        logger.error(f"Parameter ADJUSTMENT_INTERVAL:.<50: Invalid or missing, Found {adj_interval}")
    ...
    try:
        ...
        for item in expected_items:
            ...
            if inspect.isfunction(obj) or inspect.iscoroutinefunction(obj):
                is_async = 'Async' if inspect.iscoroutinefunction(obj) else 'Sync'
                logger.info(f"  {item:.<48}: {is_async} function OK")
            else:
