...
from config import open_positions, sl_prices, tp_prices, STOP_LOSS_PCT, TAKE_PROFIT_PCT, TRADE_MODE, POLL_TIMEOUT
...
async def get_position(trading_client: TradingClient, symbol: str) -> float:
    ...
    if TRADE_MODE == 'paper':
        return open_positions[symbol]['qty']  # Use in-memory state for paper mode
    try:
...
async def update_position(trading_client: TradingClient, symbol: str, side: str, trade_type: str, filled_qty: float, filled_price: float, file_handler):
    ...
    if side == 'buy':
        if trade_type == 'long':
            ...
            sl_prices[symbol] = filled_price * (1 - STOP_LOSS_PCT)
            tp_prices[symbol] = filled_price * (1 + TAKE_PROFIT_PCT)
...
async def execute_order(trading_client: TradingClient, symbol: str, qty: float, side: str, trade_type: str, current_price: float, log_file_path: str, file_handler) -> tuple[bool, str]:
    ...
    if TRADE_MODE == 'simulation' or TRADE_MODE == 'paper':
        logging.info(f"{'Simulated' if TRADE_MODE == 'simulation' else 'Paper'} {side.upper()} order (Alpaca, {trade_type}): {qty} shares of {symbol} at ${current_price:.2f}")
        success = await update_position(trading_client, symbol, side, trade_type, qty, current_price, file_handler)
        return success, ""
    else:  # TRADE_MODE == 'live'
        try:
            # Check market hours
            clock = trading_client.get_clock()
            if not clock.is_open:
                logging.warning(f"Market closed, cannot submit {side.upper()} order for {symbol} in live mode")
            ...
            success = await update_position(trading_client, symbol, side, trade_type, filled_qty, filled_price, file_handler)
            return success, ""
