...
class AMDDataProcessor:
    def __init__(self, AMD_file, spy_file, output_json='historical_analysis_AMD.json'):
        """Initialize with AMD and SPY Excel files and JSON output file."""
        self.AMD_file = AMD_file
    ...
    def load_data(self):
        """Load and clean AMD and SPY data from Excel files, assuming headers in first row."""
        # Load AMD data
        try:
            df = pd.read_excel(self.AMD_file, sheet_name='AMD', skiprows=0)
        except Exception as e:
            raise ValueError(f"Error reading AMD Excel file {self.AMD_file}, sheet AMD: {str(e)}")

        expected_columns = ['Date', 'Symbol', 'Open', 'High', 'Low', 'Close', 'Volume']
        print(f"Processing AMD sheet")
        ...
        if missing_cols:
            raise ValueError(f"AMD sheet is missing columns: {missing_cols}")

        # Handle AMD Date column (MM/DD/YYYY format)
        try:
            df['Date'] = pd.to_datetime(df['Date'], format='%m/%d/%Y', errors='coerce')
            if df['Date'].isna().any():
                invalid_dates = df[df['Date'].isna()][['Date']].head()
                print(f"AMD - Dropping {df['Date'].isna().sum()} rows with invalid dates:\n{invalid_dates.to_string()}\n")
                df = df.dropna(subset=['Date'])
        except Exception as e:
            raise ValueError(f"Error converting Date column in AMD sheet to datetime (format MM/DD/YYYY): {str(e)}\nFirst 5 Date values: {df['Date'].head().tolist()}")

        # Clean AMD numeric columns
        for col in ['Open', 'High', 'Low', 'Close', 'Volume']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
            if df[col].isna().any():
                print(f"AMD - Dropping {df[col].isna().sum()} rows with invalid values in {col}")
        ...
        print(f"AMD - First 5 rows after cleaning:\n{df.head().to_string()}\n")
        print(f"AMD - Volume for 2025-07-17: {df.loc['2025-07-17', 'Volume'] if '2025-07-17' in df.index else 'Not found'}")
        self.data = df
        ...
        # Set Date as index and align with AMD dates
        spy.set_index('Date', inplace=True)
        spy = spy[['Close']].rename(columns={'Close': 'Market_Close'})
        spy = spy.reindex(df.index, method='ffill')  # Align with AMD dates
    ...
    def run_analysis(self):
        """Run comprehensive analysis and store results for AMD."""
        self.load_data()
        df = self.data.copy()
...
if __name__ == "__main__":
    AMD_file = "/home/gmafanasiev/AMD since 2017-01-01.xlsx"
    spy_file = "/home/gmafanasiev/SPY since 2017-01-01.xlsx"
    output_json = "/home/gmafanasiev/historical_analysis_AMD.json"
    processor = AMDDataProcessor(AMD_file, spy_file, output_json)
